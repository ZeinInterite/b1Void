...
class FileManagerActivityOptimized : AppCompatActivity() {
    ...
    private fun loadDirectoryContent(directory: File) {
        ...
        thread {
            if (directory.exists() && directory.isDirectory) {
                ...
                runOnUiThread {
                    Toast.makeText(this@FileManagerActivityOptimized, "Папка не найдена", Toast.LENGTH_SHORT).show()
                }
            }
            runOnUiThread {
    ...
    private fun onFileLongClick(file: File) {
        ...
        toggleFileSelection(file)
        fileAdapter.startSwipeSelection()
    }

    private fun onSwipeSelection(file: File) {
        toggleFileSelection(file)
    }
    ...
    private fun shareFile(file: File) {
        ...
        try {
            if (file.isDirectory) {
                ...
                ZipFile(zipFile.absolutePath).addFolder(file)
    ...
    private fun clearSelection() {
        ...
        fileAdapter.stopSwipeSelection()
                            ...
                            try {
                                if (file.isDirectory) {
                                    ...
                                    if (file.delete()) {
                                        ...
                                        Log.e("File Manager", "Error deleting file ${file.name}")
                                        runOnUiThread {
                                            Toast.makeText(this@FileManagerActivityOptimized, "Не удалось удалить файл ${file.name}", Toast.LENGTH_SHORT).show()
                                        }
                                    }
                                }
                                runOnUiThread {
                                ...
                                Log.e("FileManager", "SecurityException deleting file: ${e.message}")
                                runOnUiThread {
                                    Toast.makeText(this@FileManagerActivityOptimized, "Ошибка безопасности при удалении файла ${file.name}", Toast.LENGTH_SHORT).show()
                                }
                            }
                        }
                        runOnUiThread {
                            Toast.makeText(this@FileManagerActivityOptimized, "Выбранные файлы удалены", Toast.LENGTH_SHORT).show()
                            clearSelection()
                            loadDirectoryContent(getCurrentDirectory())
                        }
                    }.start()
    ...
    private fun moveSelectedFiles(destinationDirectory: File) {
        Thread {
            ...
            selectedFiles.forEach { file ->
                ...
                try {
                    if (!file.renameTo(newFile)) {
                        errorOccurred = true
                        runOnUiThread {
                            Toast.makeText(this@FileManagerActivityOptimized, "Ошибка при перемещении файла ${file.name}", Toast.LENGTH_SHORT).show()
                        }
                    }
                } catch (e: Exception) {
                    errorOccurred = true
                    runOnUiThread {
                        Toast.makeText(this@FileManagerActivityOptimized, "Ошибка при перемещении файла ${file.name}: ${e.message}", Toast.LENGTH_SHORT).show()
                    }
                }
            }
            ...
            runOnUiThread {
                if (!errorOccurred) {
                    Toast.makeText(this@FileManagerActivityOptimized, "Файлы перемещены", Toast.LENGTH_SHORT).show()
                }
                clearSelection()
                loadDirectoryContent(getCurrentDirectory())
            }
